...
## P0.7 Jedan korisnički ulaz

Korisnik treba pokretati samo:

```text
FlowOS.exe
```

`FlowOS.exe` provjerava da li interni servis već radi, pokreće ga kada je potrebno, čeka health potvrdu i tek zatim otvara GUI.
...
ali bez zasebnih korisničkih shortcut-a.

## P0.8 Kontrolisano zatvaranje

GUI mora razlikovati:

```text
Zatvori samo prozor
Zaustavi sve i ugasi FlowOS
Odustani
```

Zatvaranje samo GUI-ja ne smije prekinuti servis, watcher, verify, session completion ni agentske procese.

Potpuno gašenje mora provjeriti aktivne sesije, zatražiti korisničku odluku i uredno zaustaviti pozadinske resurse.

## P0.9 Jedinstveni launcher/service lifecycle

Uvesti `ApplicationLifecycleService` ili odgovarajući launcher sloj koji upravlja:

- pronalaskom i pokretanjem servisa;
- health provjerom;
- sprečavanjem duplog servisa;
- ponovnim otvaranjem GUI-ja;
- system tray ponašanjem;
- graceful shutdown-om;
- recovery tokom nakon pada GUI-ja ili servisa.
...
8. Jedan korisnički launcher: FlowOS.exe
9. Skriveni interni servisni proces
10. GUI izbor: zatvori prozor ili potpuno ugasi
11. System tray ponašanje
12. Graceful shutdown aktivnih sesija i pozadinskih servisa
```
...
# 25A. Arhitektura pokretanja, procesa i gašenja

## Ključna odluka

```text
Jedna korisnička aplikacija
ne znači
jedan proces
```
...
```text
FlowOS.exe
├── jedini korisnički ulaz i GUI
├── pronalazi ili pokreće FlowOS.Service.exe
└── upravlja otvaranjem, tray režimom i gašenjem
...
Korisnik vidi samo `FlowOS.exe`. Interna procesna podjela ostaje skrivena.

## Zašto servis treba ostati zaseban proces

Ne treba spajati servis i GUI u isti proces samo radi jednog executable fajla, jer bi tada:

- pad GUI-ja mogao prekinuti agentske sesije;
- zamrznut Qt event loop mogao zaustaviti heartbeat i watcher;
- restart GUI-ja prekidao pozadinski rad;
- kvarovi i logovi bili teže izolovani;
- update GUI-ja ugrožavao aktivne agente.

Cilj je:

```text
jednostavno korisniku
otporno tehnički
```
...
```text
1. Korisnik pokrene FlowOS.exe.
2. Launcher provjeri da li servis već radi.
3. Ako ne radi, pokrene FlowOS.Service.exe.
4. Čeka /health potvrdu do definisanog timeouta.
5. Ako servis ne postane spreman, prikazuje:
   - Ponovi
   - Otvori log
   - Ugasi
6. Kada je servis spreman, otvara GUI.
7. GUI učitava projekat, sesije i resume stanje.
```

## Sprečavanje duplog servisa
...
Samo PID fajl nije dovoljan jer može ostati nakon pada procesa.
...
Ako servis već radi, novo pokretanje `FlowOS.exe` samo fokusira postojeći GUI ili ga ponovo otvara.

## Ponašanje zatvaranja prozora

Pri kliku na `X` prikazati:

```text
Šta želite uraditi?

[Zatvori samo prozor]
Pozadinski servis i agentske sesije nastavljaju raditi.

[Zaustavi sve i ugasi FlowOS]
Zaustavlja servis, watcher i aktivne sesije.
...
```text
Ne pitaj ponovo; pri zatvaranju sakrij u system tray.
```

## System tray

Meni system tray ikone:

```text
Otvori FlowOS
Aktivne sesije: N
Prikaži sesije
Pauziraj pokretanje novih sesija
Zaustavi sve sesije
...
Klik na ikonu otvara ili fokusira postojeći GUI.

## Zatvori samo GUI

```text
GUI: zatvoren ili sakriven
servis: nastavlja
agenti: nastavljaju
watcher: nastavlja
verify: nastavlja
session completion: nastavlja
```

Kada korisnik ponovo pokrene `FlowOS.exe`, GUI se povezuje na isti servis i prikazuje šta se desilo dok je bio zatvoren.

## Potpuno gašenje FlowOS-a

Predloženi tok:

```text
1. Servis prelazi u SHUTTING_DOWN.
2. Zabranjuje pokretanje novih sesija.
3. Učitava aktivne sesije i verifikacije.
4. Korisniku prikazuje izbor.
5. Čeka završetak ili prekida procese prema odluci.
6. Zaustavlja watcher.
7. Završava background taskove.
8. Flush/commit DB stanja.
9. Zatvara servis.
10. Zatvara GUI.
```

Ako postoje aktivne sesije, ponuditi:

```text
[Sačekaj završetak]
[Prekini sesije i ugasi]
[Ostavi servis aktivan, zatvori samo GUI]
[Odustani]
```

Agentske procese ne prekidati bez eksplicitne potvrde.

## Graceful shutdown API

Dodati lokalne interne endpointove:

```text
POST /system/shutdown/prepare
POST /system/shutdown/confirm
...
```json
{
  "can_shutdown": false,
  "active_sessions": 2,
  "running_verifications": 1,
  "watchers": 3,
  "options": ["WAIT", "INTERRUPT", "GUI_ONLY"]
}
```

Endpointi moraju biti dostupni samo lokalno i zaštićeni internim tokenom.
...
## Launcher odgovornosti

Launcher ne sadrži FlowOS poslovnu logiku.
...
Installer može sadržati više internih executable fajlova, ali korisnik dobija jedan shortcut.
...
```text
FlowOS
FlowOS — Otvori logove
FlowOS — Deinstaliraj
```

Ne prikazivati zasebno servis i CLI.

## Update aplikacije

Update ne smije nasilno prekinuti aktivne sesije.

```text
1. Otkriven update.
2. Ako postoje aktivne sesije, update se odlaže.
3. Kada nema aktivnog rada:
   - servis se kontrolisano gasi;
   - fajlovi se zamjenjuju;
   - servis i GUI se ponovo pokreću.
```

## Recovery nakon pada GUI-ja

- servis ostaje aktivan;
- agentske sesije nastavljaju;
- novi GUI se spaja na postojeći servis;
- Project Resume i Timeline prikazuju događaje nastale tokom odsustva.

## Recovery nakon pada servisa

- GUI prikazuje da servis nije dostupan;
- ne prikazuje stare podatke kao ažurne;
- nudi ponovno pokretanje;
- nakon restarta pokreće reconciliation;
- provjerava evidentirane agent procese;
- pokušava rekonstruisati session stanje.

## Sigurnosna pravila

- shutdown API samo localhost;
- interni token između GUI-ja i servisa;
- servis prekida samo procese koje je sam pokrenuo i evidentirao;
- proizvoljan PID iz korisničkog zahtjeva nije dovoljan;
- svaka shutdown odluka ulazi u audit i timeline.
...
USER_CANCELLED_SHUTDOWN
SESSIONS_INTERRUPTED_FOR_SHUTDOWN
SERVICE_RESTARTED
```
...
```text
test_launcher_starts_service_when_missing
test_launcher_reuses_existing_service
test_launcher_does_not_start_duplicate_service
test_launcher_reports_health_timeout
test_launcher_opens_gui_after_service_ready
```

### GUI zatvaranje

```text
test_close_gui_keeps_service_running
test_shutdown_dialog_lists_active_sessions
test_cancel_shutdown_keeps_everything_running
test_full_shutdown_stops_service
test_close_to_tray_keeps_sessions_running
```

### Servis

```text
test_shutdown_prepare_reports_active_work
test_graceful_shutdown_stops_watchers
test_graceful_shutdown_waits_for_background_tasks
test_interrupted_sessions_are_audited
```

### Recovery

```text
test_gui_reconnects_to_existing_service
test_service_restart_triggers_reconciliation
test_gui_shows_degraded_state_when_service_is_down
```

## Acceptance kriterijumi

```text
[ ] korisnik pokreće samo FlowOS.exe
[ ] servis se automatski pokreće
[ ] dupli servis nije moguć
[ ] zatvaranje GUI-ja ne prekida agentske sesije
[ ] system tray omogućava ponovno otvaranje
[ ] potpuno gašenje je eksplicitno
[ ] aktivne sesije se ne prekidaju bez potvrde
[ ] shutdown uredno zaustavlja watcher i background taskove
[ ] GUI se može ponovo povezati na postojeći servis
[ ] installer prikazuje samo jedan korisnički ulaz
```
...
```text
Jedna korisnička aplikacija
Jedan korisnički executable
Jedan interni servisni proces
Više izolovanih agentskih procesa
```

Ne težiti jednom procesu samo radi jednostavnijeg pakovanja.
...
```text
1. Pokrene samo FlowOS.exe
2. FlowOS pronađe ili automatski pokrene servis
3. Otvori projekat
4. Vidi gdje je stao
5. Vidi tačno jednu preporučenu sljedeću akciju
6. Pokrene sesiju povezanu sa plan stavkom
7. FlowOS prati heartbeat i aktivnosti
8. FlowOS otkrije konflikt
9. Sesija završi
10. FlowOS pokrene verify
11. Kreira report i evidence
12. Promijeni status plan stavke
13. Traži korisnički verdict
14. Ažurira resume
15. Ponudi sljedeću akciju
16. Korisnik zatvori samo GUI, a rad nastavi u pozadini
17. Ponovo otvori FlowOS i vidi šta se desilo
18. Po potrebi eksplicitno potpuno ugasi FlowOS
```
...
Prvo zatvoriti četiri vertikalna toka:
...
D. Launcher → Service → GUI → Background rad → Kontrolisano gašenje
```

Četvrti tok osigurava da procesna arhitektura ostane nevidljiva korisniku, dok zatvaranje GUI-ja ne prekida rad agenata.

Kada ova četiri toka budu pouzdana, FlowOS će postati stvarni lični operativni sistem za agentski razvoj, a ne samo kvalitetan preglednik razvojnih podataka.
