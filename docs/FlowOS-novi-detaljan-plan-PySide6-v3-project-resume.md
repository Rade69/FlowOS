# FlowOS — novi detaljan plan realizacije v3

**Datum:** 31. juli 2026.  
**Status:** Operativni plan za početak implementacije — dopunjen praćenjem napretka po planu, povratkom na projekat i Git reconciliation tokom  
...
26. Stanje projekta mora preživjeti zatvaranje GUI-ja, prelazak na drugi projekat i restart računara.
27. Povratak na projekat ne oslanja se na istoriju razgovora, nego na bazu, Git snapshot, report, artefakte i eksplicitni handoff.
28. FlowOS pri povratku na projekat poredi posljednje poznato Git stanje sa trenutnim stanjem i jasno označava promjene nastale van FlowOS-a.
29. Promjene nastale van FlowOS-a ne pripisuju se automatski agentu niti planiranoj stavci.
30. „Gdje si stao“ je centralna funkcija, ne dekorativni sažetak: mora imati posljednju stavku plana, status dokaza, otvorene blokade, sljedeći korak i preuslove za nastavak.
31. Jedan aktivni plan po projektu je MVP pravilo. Paralelni tipovi planova (`DEVELOPMENT`, `MIGRATION`, `SECURITY`) ne uvode se dok stvarna upotreba ne pokaže potrebu.
...
11. gdje je korisnik stao posljednji put na tom projektu;
12. šta se promijenilo van FlowOS-a od posljednje praćene sesije;
13. koji je sljedeći konkretan korak i šta mora biti provjereno prije nastavka;
14. šta korisnik treba odlučiti.
...
## 7.0A Trajno stanje projekta, povratak i reconciliation

FlowOS mora moći pouzdano odgovoriti gdje je projekat stao čak i nakon više dana, prelaska na druge projekte, zatvaranja GUI-ja ili restarta računara.
...
reconciliation_status:
CURRENT | EXTERNAL_COMMITS | EXTERNAL_DIRTY_CHANGES |
BRANCH_CHANGED | DIVERGED | REPOSITORY_MISSING |
UNKNOWN | NEEDS_REVIEW
```

`dirty_fingerprint` je stabilan hash normalizovanog `git status --porcelain=v2` izlaza. Ne koristi se kao dokaz autorstva, samo za detekciju promjene.

### ProjectResumeState

Materijalizovani sažetak za ekran „Gdje si stao“. Ne zamjenjuje izvorne podatke; može se regenerisati iz plana, sesija, reporta i Git stanja.

```text
id, project_id,
active_plan_id nullable,
last_plan_item_id nullable,
last_task_id nullable,
last_session_id nullable,
last_report_id nullable,
...
resume_status:
NO_HISTORY | READY_TO_CONTINUE | NEEDS_REVIEW |
BLOCKED | EXTERNAL_CHANGES | REPOSITORY_UNAVAILABLE

confidence:
HIGH | MEDIUM | LOW
```

`ProjectResumeState` se obnavlja nakon završetka sesije, promjene statusa plana, verifikacije, reconciliation-a i korisničke odluke. Izvor istine ostaju osnovne tabele i artefakti.

### ProjectReconciliationEvent

Append-only zapis poređenja posljednjeg poznatog i trenutnog Git stanja.
...
previous_branch,
current_branch,
previous_dirty_fingerprint,
current_dirty_fingerprint,
result,
new_commit_shas_json,
changed_files_json,
user_resolution,
occurred_at

result:
NO_CHANGE | NEW_COMMITS | DIRTY_CHANGES | BRANCH_CHANGED |
DIVERGED | REPOSITORY_MISSING | ERROR

user_resolution:
PENDING | IMPORT_AS_EXTERNAL_ACTIVITY |
LINK_TO_PLAN_ITEM | IGNORE_WITH_REASON |
CREATE_NEW_TASK | REVIEW_LATER
```

### ExternalActivity

Koristi se kada je projekat mijenjan van FlowOS-a i korisnik želi sačuvati trag bez lažne atribucije.
...
source:
MANUAL_GIT | EXTERNAL_AGENT | IDE | UNKNOWN

attribution:
UNATTRIBUTED | USER_CONFIRMED | EXTERNAL_CONFIRMED
```

### Pravila reconciliation-a

Pri otvaranju ili aktiviranju projekta:

1. učitaj posljednji `ProjectWorkspaceState`;
2. provjeri da repozitorij postoji;
3. pročitaj trenutni `HEAD`, branch i `status --porcelain=v2`;
4. uporedi sa posljednjim poznatim stanjem;
5. ako nema razlike, status je `CURRENT`;
6. ako postoje novi commitovi, status je `EXTERNAL_COMMITS`;
7. ako postoji novi dirty state, status je `EXTERNAL_DIRTY_CHANGES`;
8. ako se branch promijenio, status je `BRANCH_CHANGED`;
9. ako istorija nije direktno nastavljiva, status je `DIVERGED`;
10. ne pripisuj promjene sesiji, agentu ni planu bez korisničke potvrde;
11. zapiši `ProjectReconciliationEvent`;
12. osvježi `ProjectResumeState`.

### Pravila „Gdje si stao“
...
Project
→ ACTIVE Plan
→ posljednji relevantni PlanItem
→ posljednja AgentSession
→ posljednji AgentReport
→ GitSnapshot / ProjectWorkspaceState
→ Verification rezultat
→ otvorene blokade i odluke
→ reconciliation rezultat
```
...
FlowOS ne smije prikazati „spremno za nastavak“ ako postoji neobrađen reconciliation događaj ili nejasan dirty tree.
...
```text
id, name, repo_path, status, notes, last_opened_at, created_at, updated_at
```
...
where_stopped, next_concrete_step, resume_preconditions,
...
## 8.2A Povratak na projekat i reconciliation

```text
GET    /projects/{project_id}/resume
POST   /projects/{project_id}/resume/regenerate
POST   /projects/{project_id}/reconcile
GET    /projects/{project_id}/reconciliation-events
POST   /projects/{project_id}/reconciliation-events/{event_id}/resolve

GET    /projects/{project_id}/workspace-state
POST   /projects/{project_id}/external-activities
GET    /projects/{project_id}/external-activities
```

`POST /reconcile` ne mijenja plan ni atribuciju automatski. Vraća razliku i predložene korisničke opcije.

Resolve akcije:

```text
IMPORT_AS_EXTERNAL_ACTIVITY
LINK_TO_PLAN_ITEM
CREATE_NEW_TASK
IGNORE_WITH_REASON
REVIEW_LATER
...
`LINK_TO_PLAN_ITEM` traži korisničku potvrdu i ne smije retroaktivno izmišljati autora promjene.
...
project.reconciliation.created
project.reconciliation.resolved
external_activity.created
session.created
session.updated
session.completed
...
12. AgentReport obavezno navodi gdje je rad stao, sljedeći konkretan korak i preuslove za nastavak.
13. ProjectResumeService regeneriše `ProjectResumeState`.
14. ProjectWorkspaceState se ažurira završnim Git stanjem.
15. Status postaje `COMPLETED` ili `NEEDS_REVIEW`.
...
## 10.5 Promjena aktivnog projekta

Kada korisnik izabere drugi projekat u GUI-ju:

1. Controller učitava sažetak iz baze bez čekanja na Git;
2. View odmah prikazuje posljednje poznato stanje sa oznakom vremena i confidence nivoom;
3. ProjectReconciliationService u pozadini provjerava trenutni Git state;
4. ako nema razlike, oznaka postaje `Ažurno`;
5. ako postoje vanjske promjene, GUI prikazuje upozorenje i ne prepisuje prethodni sažetak;
6. korisnik bira kako će promjene evidentirati;
7. nakon odluke regeneriše se „Gdje si stao“.

GUI thread se ne blokira Git provjerom.

## 10.6 Projekat mijenjan bez FlowOS-a

FlowOS mora jasno razlikovati:

```text
praćena sesija
→ poznat agent, task, PlanItem i dokazi

vanjska aktivnost
→ Git dokaz postoji, ali autor i namjera nisu potvrđeni
```

Moguće korisničke akcije:

- pregledaj commitove i fajlove;
- uvezi kao vanjsku aktivnost;
- poveži sa postojećom planiranom stavkom;
- napravi novi task;
- ostavi za kasniji review;
- ignoriši uz obavezno obrazloženje.

Ignorisanje ne briše `ProjectReconciliationEvent`.
...
## 13.9 Projekti i „Gdje si stao“

### Ekran Projekti

Svaka projektna kartica ili red prikazuje:

```text
Naziv projekta
Posljednji rad: datum/vrijeme
Aktivni plan i posljednja stavka
Status: READY / BLOCKED / NEEDS_REVIEW / EXTERNAL_CHANGES
Posljednji commit
Broj otvorenih blokada
Broj neriješenih vanjskih promjena
```

### Panel „Gdje si stao“

Za aktivni projekat prikazuje aktivni plan, posljednju stavku, stanje, posljednju sesiju, posljednji dokaz, otvorene probleme, sljedeći konkretan korak i šta mora biti provjereno prije nastavka.

### Reconciliation upozorenje

Ako je projekat mijenjan van FlowOS-a, prikazati nove commitove, nekomitovane fajlove i promjenu brancha, sa akcijama pregledaj, uvezi, poveži sa planom ili kasnije. Dok upozorenje nije riješeno, confidence ne može biti `HIGH`.
...
View ne izvršava Git i ne računa resume status.

## 13.10 Napredak po planu
...
## 19.4 Handoff za povratak na projekat

Svaki netrivijalan report mora sadržati:

```markdown
## Gdje je rad stao
<<< posljednje pouzdano stanje, ne opšti sažetak >>>

## Sljedeći konkretan korak
<<< jedna konkretna akcija koja prirodno nastavlja rad >>>

## Šta mora biti provjereno prije nastavka
<<< dirty tree, neuspjeli test, otvorena odluka, neobrađen diff ili "Nema." >>>
```

Ova polja hrane `ProjectResumeState`. Agent ne smije napisati „nastaviti dalje“ bez konkretne akcije.

## 19.5 Usklađenost AgentReporta sa planom
...
#### FLOW-103D — Project resume model i Service

- ProjectWorkspaceState;
- ProjectResumeState;
- ProjectReconciliationEvent;
- ExternalActivity;
- regeneracija resume sažetka iz trajnih izvora;
- confidence pravila;
- unit i temp DB testovi;
- bez Git reconciliation-a u ovom zadatku.
...
#### FLOW-104A — Project resume API

- `/projects/{id}/resume`;
- `/projects/{id}/resume/regenerate`;
- workspace state read;
- external activity create/list;
- contract testovi;
- nema Git komandi u API Controlleru.
...
#### FLOW-105B — Projects i „Gdje si stao“ GUI skeleton

- `ProjectsView`;
- `ProjectResumeView`;
- `ProjectsController`;
- `ProjectResumeController`;
- prikaz posljednjeg poznatog sažetka;
- confidence i timestamp;
- loading/empty/error stanja;
- bez Git reconciliation-a u GUI procesu.
...
- projekat prikazuje posljednje poznato „Gdje si stao“ stanje nakon restarta;
- resume sažetak sadrži posljednju stavku, report, sljedeći korak i preuslove;
...
#### FLOW-205A — ProjectReconciliationService

- poređenje posljednjeg poznatog i trenutnog HEAD/branch/status stanja;
- detekcija novih commitova;
- detekcija novog dirty state-a;
- branch changed i diverged slučajevi;
- append-only reconciliation event;
- bez automatske atribucije;
- integration testovi na privremenom Git repou.

#### FLOW-205B — Reconciliation API i WebSocket

- pokretanje reconciliation-a;
- pregled događaja;
- resolve akcije;
- WebSocket događaji;
- idempotentno rješavanje;
- optimistic locking za korisničku odluku.
...
#### FLOW-207B — Reconciliation GUI

- upozorenje pri povratku na projekat;
- pregled novih commitova i dirty fajlova;
- akcije import/link/create task/later/ignore with reason;
- confidence downgrade dok događaj nije riješen;
- Git rad se izvršava kroz backend Service;
- controller testovi toka odluke.
...
- panel `Gdje si stao` sa posljednjim dokazom, otvorenim problemom i sljedećim korakom;
- upozorenje ako reconciliation nije riješen.
...
- povratak na projekat detektuje nove commitove, dirty tree i promjenu brancha;
- FlowOS ne pripisuje vanjske promjene agentu bez korisničke potvrde;
- „Gdje si stao“ se osvježi nakon reconciliation odluke.
...
- svaki netrivijalan report ima konkretan handoff za povratak;
- ProjectResumeState se regeneriše nakon reporta i verifikacije.
...
## Project resume i reconciliation

- projekat bez prethodne istorije;
- povratak bez Git promjena;
- novi commitovi van FlowOS-a;
- dirty tree van FlowOS-a;
- promijenjen branch;
- diverged istorija;
- repo premješten ili obrisan;
- nevalidan/missing posljednji snapshot;
- resolve kao ExternalActivity;
- link sa PlanItem;
- create new Task;
- ignore with reason;
- neriješen event spušta confidence;
- restart GUI-ja zadržava isti resume;
- završetak sesije regeneriše resume;
- report bez sljedećeg koraka pada validaciju za netrivijalan zadatak.
...
18. Vrijeme da korisnik utvrdi „gdje sam stao na ovom projektu“: cilj <10 sekundi.
19. Udio netrivijalnih reporta sa konkretnim sljedećim korakom i preuslovima: cilj 100%.
20. Broj neriješenih reconciliation događaja starijih od 7 dana.
21. Udio vanjskih promjena koje su pogrešno atribuirane agentu: cilj 0.
22. Udio projekata čiji resume ima `HIGH` confidence nakon otvaranja: mjeriti, ne postavljati cilj prije stvarne upotrebe.
...
| Automatska semantička rekonstrukcija namjere iz Git diff-a | nepouzdano zaključivanje poslovne namjere | tek nakon dovoljnog označenog skupa i uvijek kao prijedlog |
| Više paralelnih aktivnih planova po projektu | povećava dvosmislenost task/sesija/status veza | stvarna potreba za `plan_type` |
| Automatsko pripisivanje vanjskih commitova agentu | Git ne dokazuje autora agentske sesije | nikad bez pouzdanog adapter dokaza ili korisničke potvrde |
...
7. popuni `Gdje je rad stao`, `Sljedeći konkretan korak` i `Šta mora biti provjereno prije nastavka`;
8. predloži status najviše `IMPLEMENTED`; ne proglašavaj `VERIFIED` ili `ACCEPTED`;
9. navedi šta nije provjereno;
10. za HIGH/CRITICAL obezbijedi nezavisnog checkera;
11. ne počinji sljedeći task bez korisničke potvrde ako task mijenja arhitekturu ili scope plana.
...
GDJE JE RAD STAO: konkretno stanje
SLJEDEĆI KORAK: jedna konkretna akcija
PRIJE NASTAVKA PROVJERITI: lista | Nema
...
13. gdje je projekat stao posljednji put;
14. da li je projekat u međuvremenu mijenjan van FlowOS-a;
15. koji je sljedeći konkretan korak;
16. šta mora biti provjereno prije nastavka;
17. koju odluku korisnik sada treba donijeti.
