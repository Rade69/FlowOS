...
#### FLOW-1104 — Uvesti ovaj plan u FlowOS i potvrditi PlanImport tok

**Rizik:** MEDIUM

Nakon stabilnog LIVE starta uvesti ovaj Markdown plan kroz postojeći PlanImport tok i koristiti ga kao osnovu za naredne faze.
...
**Dokaz:** Plan faza 11–15 je vidljiv u LIVE FlowOS-u i može se koristiti za praćenje narednog rada.
...
Završiti FLOW-1104 prije FLOW-1201.
...
3. `REVIEW_COMPLETED` prikazati kao završen review, ne kao prihvatanje.
...
2. `REVIEW_COMPLETED` može otvoriti povezani review report ako postoji.
3. `TEST_RESULT` može otvoriti ili prikazati metadata verification artifacta.
...
Obavezno:
1. `Prihvati rezultat` mapira se na `ACCEPTED`.
2. `Vrati u doradu` mapira se na `NEEDS_WORK`.
3. `Odbaci rezultat` mapira se na `REJECTED`.
