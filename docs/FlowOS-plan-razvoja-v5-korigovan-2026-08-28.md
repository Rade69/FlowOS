...
## 0.1 Polazni snapshot na fiksiranom SHA-u

Ovaj roadmap polazi od stvarnog stanja, ne od praznog projekta:

```text
FLOW-1109 secret redaction       → završeno / commitovano
FLOW-1110 worktree path safety   → otvoreno; tri prefix provjere potvrđene
Plan Import                      → GUI/backend contract mismatch
Session start                    → pasivna registracija; legacy launch semantika još postoji
EvidenceService                  → postoji, trenutno PlanItem-keyed
ProjectStateService              → postoji
Project/session timeline servisi → postoje
Workflow Ledger + TASK_DECISION  → postoje kao dva canonical writer puta
TaskContract                     → ne postoji kao model
file claims                      → ne postoje
FlowOS MCP/read-only agent API   → ne postoji
GUI                              → 10 stranica; Zadaci nisu wired; Aktivnost je widget na Pregledu
architecture enforcement         → guard skripta + architecture testovi, ali sa različitim coverageom
```

Zato v5 prvenstveno **ispravlja ugovore, povezuje postojeće primitive i mehanički provodi blueprint jezgro**. Ne kreće od pretpostavke da sve treba graditi iznova.
...
Na fiksiranom SHA-u postoje **tri** unsafe string-prefix provjere u `worktrees/service.py`:

```python
# list_flowos_worktrees()
...
Sva tri mjesta ulaze u obim FLOW-1110.

Dva slučaja (`list_flowos_worktrees` i `_dict_to_info`) pokušavaju dokazati containment pripadnost FlowOS worktree rootu preko string prefiksa. `_find_worktree` pokušava dokazati identity preko equality-or-prefix uslova. Nijedna od te dvije semantike ne smije ostati zasnovana na string `startswith`.
...
[ ] sva tri potvrđena prefix mjesta (list, find, is_main) koriste centralnu path semantiku
[ ] root containment ne koristi string prefix
[ ] worktree identity ne koristi prefix fallback
...
Postoje **dva enforcement puta** koja se moraju uskladiti:
...
Na fiksiranom SHA-u `scripts/verify.py` pokreće `pytest tests/architecture/`; **ne pokreće direktno `scripts/guard_architecture.py`**. Zato FLOW-1156 ne smije pretpostaviti da je PASS jednog puta dokaz za drugi.

Prije prihvatanja oba puta se replayuju protiv poznatog starog stanja i oba moraju dokazivo uhvatiti najmanje:
...
Guard/test koji “prođe” jer su prekršaji allowlistovani nije prihvaćen.
...
[ ] guard skripta i architecture test suite oba hvataju sva tri poznata stara prekršaja
[ ] `scripts/verify.py` je zelen, uz eksplicitno zabilježeno da trenutno pokreće architecture testove, ne guard skriptu
...
> **v5 napomena o primjenjivosti za zaštićeni §22:** tekst §22 ispod je namjerno ostavljen byte-identičan v4.3 po change specu. U **Fazi C / Milestone 1** izraz „Zadaci — glavna radna površina“ ostvaruje se kroz board svih aktivnih Taskova. Rečenica „Task Detail — centralna jedinica razumijevanja i odluke“ opisuje **kasniji post-Milestone-1 target** ako FLOW-1204 bude aktiviran iz Faze E. Isto važi za obavezni reuse FLOW-1200 primitive-a: to nije Gate C uslov dok FLOW-1200 ostaje `ODGOĐENO`.
...
---
> **v5 napomena o primjenjivosti za zaštićeni §33:** §33 je sačuvan byte-identično iz v4.3. Podsekcije **Task Detail** i **GUI primitives** zato predstavljaju buduću test matricu za FLOW-1204/FLOW-1200 kada budu aktivirani iz Faze E; **nisu acceptance kriteriji Faze C**. Gate C koristi board-first testove iz §10: svi aktivni Taskovi na jednom ekranu, odvojeni mehanički signal i workflow stanje, tišina, „čeka mene“, decision i canonical reload.
