...
class OverviewController(QObject):
    ...
    def load_projects(self): self._api.get_projects()
    def load_plan_progress(self, pid: str): self._api.get_plan_progress(pid)
    def load_resume(self, pid: str): self._api.get_resume(pid)
    def load_sessions(self, pid: str): self._api.get_active_sessions(pid)
    def check_health(self): self._api.check_health()
    ...
    def _on_projects(self, data: list):
        self.projects_loaded.emit([{"id": p.get("id",""), "name": p.get("name",""), "repo_path": p.get("repo_path",""), "status": p.get("status","ACTIVE")} for p in data])

    def _on_plan_progress(self, data: dict):
        if "error" in data: self.error_occurred.emit(data["error"]); return
        plan = data.get("plan")
    ...
    def _on_sessions(self, data: list):
        self.sessions_loaded.emit([{
            "id": s.get("id",""), "agent_type": s.get("agent_type",""),
            "plan_item_id": s.get("plan_item_id"), "status": status_label(s.get("status","")),
            "started_at": s.get("started_at",""), "worktree_path": s.get("worktree_path"),
