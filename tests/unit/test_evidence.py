...
class TestEvidenceBundle:
    ...
    def test_evidence_bundle_with_commit_and_verification(self, engine):
        ...
        from flowos.service.services.infrastructure.persistence.models import (
            AgentSession, Project, SessionEvent,
        )
        from flowos.service.services.infrastructure.persistence.plan_models import (
            Plan, PlanItem, PlanPhase,
        )
        ...
        try:
            ...
            db.flush()
            db.add_all([
                AgentSession(id=sid, project_id=pid, plan_item_id=itid,
                             agent_type="cc", repo_path="/tmp/test",
                             base_commit_sha="abc123", result_commit_sha="def456",
                             status="COMPLETED"),
                SessionEvent(session_id=sid, event_type="VERIFY_RESULT",
                             summary="Verify PASS", source="VERIFICATION_SERVICE",
    ...
    def test_evidence_bundle_with_report(self, engine):
        ...
        try:
            ...
            db.add_all([
                AgentSession(id=sid, project_id=pid, plan_item_id=itid,
                             agent_type="pi", repo_path="/tmp/test", status="COMPLETED"),
                AgentReport(id=rid, session_id=sid, summary="Done",
                            user_verdict="ACCEPTED", status="FINAL"),
            ])
            db.commit()
    ...
    def test_evidence_bundle_excludes_unrelated_conflicts(self, engine):
        ...
        try:
            ...
            db.add_all([
                AgentSession(id=sid, project_id=pid, plan_item_id=itid,
                             agent_type="cc", repo_path="/tmp/test", status="COMPLETED"),
                Conflict(id=cid, project_id=pid, conflict_type="WRITE_WRITE",
                         conflict_level="CRITICAL", description="Test conflict",
                         conflict_key=f"write_write::{pid}::unrelated/file.py",
                         file_path="unrelated/file.py", status="OPEN",
                         evidence_json="{}", first_seen_at=now, last_seen_at=now,
                         occurrence_count=1),
            ])
            db.commit()
    ...
    def test_evidence_bundle_handles_multiple_sessions(self, engine):
        ...
        try:
            ...
            db.flush()
            db.add_all([
                AgentSession(id=oid, project_id=pid, plan_item_id=itid,
                             agent_type="cc", repo_path="/tmp/test", status="COMPLETED",
                             started_at=datetime(2026, 1, 1, tzinfo=UTC)),
                AgentSession(id=nid, project_id=pid, plan_item_id=itid,
                             agent_type="pi", repo_path="/tmp/test", status="COMPLETED",
